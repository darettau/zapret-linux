"""AES-CTR. Uses `cryptography` if installed, otherwise a ctypes wrapper over
the system OpenSSL `libcrypto`. Exposes the subset used here:
    ...
    def _load_libcrypto():
        ...
        for c in candidates:
            try:
                ...
                last_err = e
        raise RuntimeError(
            "libcrypto not found; install openssl. Last error: %r" % last_err
        )
